...
```shell
pacman -S --needed cuda ffnvcodec-headers libva-nvidia-driver nvidia-cg-toolkit nvidia-settings nvidia-utils nvidia-open-dkms nvtop opencl-nvidia openssl-1.1 gst-plugins-bad ollama-cuda
```

Note: The `openssl-1.1` package is a (missing) dependency for
`libnvidia-pkcs11.so` contained in `nvidia-utils`.

Note 2: There is also `cuda-tools` with some extra CUDA related development
utilities, but it depends on (ancient) Java 8; I chose not to install it.
