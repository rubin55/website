...
# Git clone them all.
cd "$PKG_ROOT/Build"
export GITREPOS="adwaita-qt-git aic94xx-firmware akku alsa-hdspeconf amitools ares-emu aseprite ast-firmware attract-git audacious-gtk3 audacious-plugins-gtk3 aurutils battop-bin blastem-hg bluez-hcitool brscan4 bun-bin bundletool chez-scheme clojure-lsp-bin cmake-format cmake-language-server codechecker commitlint conan cubeb displaylink djmount dockerfile-language-server dolphin-emu-git dosbox-x dotnet-core-bin drawio-desktop-bin duckstation-git dumpasn1 earthly eclipse-java erlang_ls evdi-dkms exercism figma-linux-bin flow-bin flutterup flycast-git ghcup-hs-bin github-desktop-bin godot-mono-bin google-cloud-cli groovy-language-server-git hydroxide ibmcloud-cli icaclient imhex infer-bin irccloud-bin jdk17-jetbrains-bin jdk17-openj9-bin jdk21-jetbrains-bin jdk21-openj9-bin jdtls jetbrains-toolbox kotlin-language-server-bin kubelogin lagrange lemminx lexical-bin lib32-gperftools lib32-intel-gmmlib lib32-intel-media-driver libbacktrace-git librashader libretro-beetle-cygne-git libretro-beetle-lynx-git libretro-beetle-ngp-git libretro-beetle-pcfx-git libretro-bluemsx-git libretro-dosbox-pure-git libretro-swanstation-git libretro-uae-git libspng-git libwireplumber-4.0-compat license-wtfpl logisim-evolution-bin m64py marksman-git marsdev-git mednaffe mei-amt-check-git metals mkinitcpio-firmware mongodb moonlight-qt ms-sys ncurses5-compat-libs neovim-symlinks nestopia netcoredbg networkmanager-iwd nodejs-husky nvidia-patch-git ocenaudio-bin omnisharp-roslyn-bin openmsx openshift-client-bin openshift-codeready-bin openshift-developer-bin openshift-pipelines-bin papirus-folders passmark-performancetest-bin pcsx2-git pegasus-frontend-git perlnavigator postman2openapi-bin postman-bin powercap powershell-bin pragmatapro-fonts protonplus protontricks ps3-disc-dumper-bin pupdate-bin pvsneslib-git pwvucontrol python-dataclasses-json python-m3u8 python-machine68k python-mpegdash python-patch-ng python-pathvalidate python-pdm-pep517 python-pluginbase python-pyqtdarktheme-fork python-pysdl2 python-pytest-runner python-python-ffmpeg python-tidalapi python-vdf qt5-gamepad rabtap rcu-bin regionset roomeqwizard rpcs3-git ruby-backport ruby-e2mmap ruby-jaro_winkler ruby-reverse_markdown ruby-solargraph rusty-psn-bin ryujinx sameboy scala-dotty scala-scala3-symlink scheme-chez-symlink sedutil shellcheck-bin sigtop-git skyscraper-git slack-desktop snd-hdspe-dkms soapui softhsm-git sublime-text-4 sunshine-git swift-bin tidal-dl-ng tidal-hifi-bin tla-toolbox townsemu-git ttf-b612 ungoogled-chromium-bin upd72020x-fw v4l-utils-git vala-language-server vasm visual-studio-code-bin vlc-git vlink vmware-horizon-client vmware-keymaps vtsls wd719x-firmware wxedid xcursor-dmz xdg-terminal-exec-git xpadneo-dkms yaak-bin yaml-language-server-git y-cruncher zeal-git zoom"
...
I maintain these AUR packages:
  ...
  * [nodejs-husky](https://aur.archlinux.org/packages/nodejs-husky)
